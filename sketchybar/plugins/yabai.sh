...
set_icon() {
  ...
  read -r FLOATING SPLIT FULLSCREEN STACK_INDEX <<<"$(echo "$WINDOW" | jq -rc '.["is-floating", "split-type", "has-fullscreen-zoom", "is-sticky", "stack-index"]')"
...
case "$SENDER" in
"mouse.clicked" | "alfred_trigger") # TODO
  mouse_clicked
  ;;
