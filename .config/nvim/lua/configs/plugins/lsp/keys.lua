return {
  setup = function(client, buffer)
    ...
    local lsp_map = function(key, func, id, desc)
      key_map.map("n", key, func, bufopts, "LSP", id, "LSP: " .. desc)
    end
    ...
    -- LSP Saga
    -- key_map.map({ "n", "v" }, "<leader>ca", "<cmd>:Lspsaga code_action<cr>", { silent = true }, "LSP",
    --   "lspsaga_code_action", "LSP: Code Action")
