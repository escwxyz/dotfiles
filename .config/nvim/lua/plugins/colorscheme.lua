...
local function select_theme(prompt_bufnr)
    ...
    -- NOTE you cannot change the init.lua unless also update the linenr
    sad("05", vim.g.Theme, selected[1], init_path)
...
M.find_themes = function()
    local picker = pickers.new(dropdown, {
        ...
        attach_mappings = function(_, map) -- NOTE the map function does not supports multiple modes
